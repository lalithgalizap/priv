...
export const POST = dispatchEnvelope({
  ...
  PATCH: async (ctx) => {
    const url = new URL((ctx.request as NextRequest).url);
    const profileId = url.searchParams.get("id") || (ctx.body as { id?: string })?.id;
    if (!profileId) ctx.fail(400, "Missing profile id.");
    const res = await callBackend({
      method: "PATCH",
      path: `/api/v1/org/members/${encodeURIComponent(profileId!)}/role`,
      authHeader: ctx.authHeader,
      body: ctx.body,
    });
    if (!res.ok) ctx.fail(res.status, (res.data as { error?: string })?.error || `HTTP ${res.status}`);
  ...
  DELETE: async (ctx) => {
    ...
    const res = await callBackend({
      method: "DELETE",
      path: `/api/v1/org/members/${encodeURIComponent(id!)}`,
      authHeader: ctx.authHeader,
    });
    if (!res.ok) ctx.fail(res.status, (res.data as { error?: string })?.error || `HTTP ${res.status}`);
    return res.data;
